...
const Customizer = () => {
  ...
  const thColors: { id: number, bgColor: string, disp: string }[] = [
    {
      id: 1,
      bgColor: '#5D87FF',
      disp: 'BLUE_THEME',
      ...
      >
        ...
        <Scrollbar sx={{ height: 'calc(100vh - 5px)' }}>
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Stack sx={{ direction: 'row', gap: 2, my: 2 }}>
              <StyledBox onClick={() => dispatch(setDarkMode('light'))} sx={{ display: "flex", gap: 1 }}>
                <WbSunnyTwoToneIcon
                  color={customizer.activeMode === 'light' ? 'primary' : 'inherit'}
                />
                Light
              </StyledBox>
              <StyledBox onClick={() => dispatch(setDarkMode('dark'))} sx={{ display: "flex", gap: 1 }}>
                <DarkModeTwoToneIcon
            ...
            <Stack sx={{ direction: 'row', gap: 2, my: 2, }}>
              <StyledBox onClick={() => dispatch(setDir('ltr'))} sx={{ display: "flex", gap: 1 }}>
                <SwipeLeftAltTwoToneIcon
                  color={customizer.activeDir === 'ltr' ? 'primary' : 'inherit'}
                />{' '}
                LTR
              </StyledBox>
              <StyledBox onClick={() => dispatch(setDir('rtl'))} sx={{ display: "flex", gap: 1 }}>
            ...
            {/* ------------ Theme Color setting ------------- */}
            {/* ------------------------------------------- */}
            <Typography sx={{ variant: "h6", mb: 1 }} gutterBottom>
              Theme Colors
            </Typography>
            ...
            <Stack sx={{ direction: 'row', gap: 2, my: 2 }}>
              <StyledBox onClick={() => dispatch(toggleHorizontal(false))} sx={{ display: "flex", gap: 1 }}>
                <ViewComfyTwoTone
                  color={customizer.isHorizontal === false ? 'primary' : 'inherit'}
                />
                Vertical
              </StyledBox>
              <StyledBox onClick={() => dispatch(toggleHorizontal(true))} sx={{ display: "flex", gap: 1 }}>
            ...
            <Stack sx={{ direction: 'row', gap: 2, my: 2, }}>
              <StyledBox onClick={() => dispatch(toggleLayout('boxed'))} sx={{ display: "flex", gap: 1 }}>
                <CallToActionTwoToneIcon
                  color={customizer.isLayout === 'boxed' ? 'primary' : 'inherit'}
                />
                Boxed
              </StyledBox>
              <StyledBox onClick={() => dispatch(toggleLayout('full'))} sx={{ display: "flex", gap: 1 }}>
            ...
            {customizer.isHorizontal ? (
              ...
              <>
                ...
                <Stack sx={{ direction: 'row', gap: 2, my: 2, }}>
                  <StyledBox onClick={() => dispatch(toggleSidebar())} sx={{ display: "flex", gap: 1 }}>
            ...
            <Stack sx={{ direction: 'row', gap: 2, my: 2, }}>
              <StyledBox onClick={() => dispatch(setCardShadow(false))} sx={{ display: "flex", gap: 1 }}>
                <BorderOuter color={!customizer.isCardShadow ? 'primary' : 'inherit'} />
                Border
              </StyledBox>
              <StyledBox onClick={() => dispatch(setCardShadow(true))} sx={{ display: "flex", gap: 1 }}>
